...
options = ['Distilbert-Base', 'roBERTa-large', 'Linear Regression', 'Naive Bayes', 'Gradient Boosted Classifier']
...
def predict_label_classical(text, model_name):
    ...
    if model_name == 'gbc':
        model_path = os.path.join(os.getcwd(), 'Weights\\GBC\\')
        with open(model_path+'gbc.pkl', 'rb') as file:
            model = joblib.load(file)
            print("Loaded model successfully!") 

        with open(model_path+'gbc_vect.pkl', 'rb') as file:
...
if st.button('Predict'):
    ...
    if selected_option == "Naive Bayes":
        ...
        st.write('Predicted Label:', prediction)
    if selected_option == "Gradient Boosted Classifier":
        prediction = predict_label_classical(input_text, 'gbc')
        st.write('Predicted Label:', prediction)